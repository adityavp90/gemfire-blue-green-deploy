...
printf "\n"
printf "### Adding data from blue-app (localhost:8081) - Will store objects which have just the name field\n"
curl -X PUT localhost:8081/person/1
curl -X PUT localhost:8081/person/2
...
deploy --jar=./target/functions-1.0-SNAPSHOT.jar
...
curl -X PUT localhost:8081/person/3
curl -X PUT localhost:8081/person/4
...
curl -X PUT localhost:8082/person/5
curl -X PUT localhost:8082/person/6
...
curl -X PUT localhost:8082/person/1
curl -X PUT localhost:8082/person/7
